...
void machineStep(){
	...
	/* Check for overrun */
	if (OverrunFlag) {
		rtmSetErrorStatus(Odemetrie_M, "Overrun");
		return;
	}
	...
	msgOdemetrie->set_phi_kon(Odemetrie_Y.Phi_kon); // '<Root>/Phi_kon'
	msgOdemetrie->set_y_dis_roi(Odemetrie_Y.Y_dis_roi);      // '<Root>/Y_dis_roi'
	msgOdemetrie->set_x_dis_roi(Odemetrie_Y.X_dis_roi);      // '<Root>/X_dis_roi'
	msgOdemetrie->set_center_dresch(Odemetrie_Y.center_Dresch);  // '<Root>/center_Dresch'
	msgOdemetrie->set_x_dis_glo(Odemetrie_Y.X_dis_glo);      // '<Root>/X_dis_glo'
	msgOdemetrie->set_y_dis_glo(Odemetrie_Y.Y_dis_glo);      // '<Root>/Y_dis_glo'
	msgOdemetrie->set_x_kon_roi(Odemetrie_Y.X_kon_roi);      // '<Root>/X_kon_roi'
	msgOdemetrie->set_x_kon_glo(Odemetrie_Y.X_kon_glo);      // '<Root>/X_kon_glo'
	msgOdemetrie->set_y_kon_roi(Odemetrie_Y.Y_kon_roi);      // '<Root>/Y_kon_roi'
	msgOdemetrie->set_y_kon_glo(Odemetrie_Y.Y_kon_glo);	// '<Root>/Y_kon_glo'
...
void getMachineModelData(rsb::EventPtr canEvent){
	...
	if (canId ==  cVdtcShlcVehicleSpeedMta_ID){
		...
		if(rtmGetErrorStatus(Odemetrie_M) == (NULL)){
...
int main(int argc, char **argv) {
	...
	("offset,c", po::value < double > (&Odemetrie_U.Offsetcurvature), "Offset curvature for machine, dafault = 0.0226")
	("diskretAuf,d", po::value < double > (&Odemetrie_U.Diskrete_Auf), "Discrete resolution for roi = 0.05");
