...
// Translate a map
template <typename T>
void translateMap(cv::Mat &src, cv::Mat &dst, double offsetx = 0, double offsety = 0, T fillValue = claasNumeric::invalidValue_int16) {
...
void mapRefreshAndStorage(std::shared_ptr<std::vector<cv::Mat>> mapStack,
                          ...
                          T fillValue = claasNumeric::invalidValue_int16) {
...
void getStatistic(const std::vector<cv::Mat> &src, cv::Mat &dst, const statistics statisticReq, const uint8_t quantil = 0/*1 .. 99 %*/) {
  ...
  if (statisticReq == statistics::variance || statisticReq == statistics::quantil) {
    for (std::size_t idxMap = 0; idxMap < src.size(); ++idxMap) {
      for (int idxTile = 0; idxTile < src.at(idxMap).rows * src.at(idxMap).cols; ++idxTile) {
        ...
        if (currentVal != claasNumeric::invalidValue_int16) {
  ...
  for (int idxTile = 0; idxTile < src.at(0).rows * src.at(0).cols; ++idxTile) {
    if (validCounter.at<int32_t>(idxTile) != 0) {
      ...
      dstTmp.at<int32_t>(idxTile) = claasNumeric::invalidValue_int32;
    }
  }
...
///
void getStatisticOutput(const claas_msgs::mapPrimitive &view, claas_msgs::rsm &output, const std::shared_ptr<std::vector<cv::Mat>> &input, const statistics stat, const std::string &outputDimension) {
...
void tfTileNameHandler(const std_msgs::String nameMsg) {
  ...
  if (currentTileTfNameChange) {
    ...
    mapRefreshAndStorage( lastPulsewidth_ps,                     // Map to shift/store/reset
                          ...
                          claasNumeric::invalidValue_int16);     // Fill-up value
...
int main(int argc, char **argv)
{
  ...
  n.param<std::string>("topic_laser", topicLaser, scopes::lase1);
...

  {
    ...
    cv::Mat mapInit(mapSizeX_tiles, mapSizeX_tiles, CV_16SC1, cv::Scalar_<int16_t>(claasNumeric::invalidValue_int16));
