...
static double lower_boundary_p, upper_boundary_p, lower_boundary_m, upper_boundary_m, fuzzyUpperLimit_m, fuzzyScale;
static int debug;
...
void callback(const sensor_msgs::LaserScan::ConstPtr &msg) {
  ...
  for (auto it = cloudBase.points.begin()+1; it != cloudBase.points.end()-1; ++it) {
      if (fabs(it->x) < (width / 2.0) && fabs(it->y) < (depth / 2.0)) {
          const float zDiff = fmax(fabs(it->z - (it-1)->z), fabs(it->z - (it+1)->z));
          // Do symmetric fuzzy classification with respect to the boundaries
          char ogmVal;
          if (zDiff < lower_boundary_m) {
              ogmVal = lower_boundary_schar;
          } else if (zDiff > upper_boundary_m) {
              ogmVal = upper_boundary_schar;
          } else {
              ogmVal = char(fuzzyScale * (zDiff - lower_boundary_m));
          }
...
int main(int argc, char **argv){
  ...
  n.param<double>("upper_boundary_m", upper_boundary_m, 0.05);
  n.param<double>("lower_boundary_m", lower_boundary_m, 0.01);
  n.param<double>("upper_boundary_p", upper_boundary_p, 0.8);
  n.param<double>("lower_boundary_p", lower_boundary_p, 0.2);
  n.param<int>("debug", debug, 0); // Enable debug outputs

  upper_boundary_schar = schar(upper_boundary_p * 100.0);
  lower_boundary_schar = schar(lower_boundary_p * 100.0);
  fuzzyScale = (upper_boundary_schar - lower_boundary_schar) / (upper_boundary_m - lower_boundary_m);
