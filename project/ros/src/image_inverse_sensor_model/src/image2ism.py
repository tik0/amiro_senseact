...
topicOutName = rospy.get_param(nodeName+'/topicOut', '') # 1000 is not specified. 0-19 is pascal classes. 20 is the pedestrian detector
imageWidth = rospy.get_param(nodeName+'/imageWidth', 800) 
imageHeight = rospy.get_param(nodeName+'/imageHeight', 600) 
object_extend = rospy.get_param(nodeName+'/object_extend', 600) 
...
rowHorizonOffset = rospy.get_param(nodeName+'/row_horizon_offset', 0)
parent_frame = rospy.get_param(nodeName+'/parent_frame', 'base_link')
...
# Add an offset
rHorizon = int(rHorizon + rowHorizonOffset)

topics = topicsInName.split(' ')
print "Input:  " + topicsInName
print "Output: " + topicOutName
print "rHorizon: "
print rHorizon
pubImageObjs = list()
...
for iTopic in range(0,len(topics)): 
    ...
    topicOutNameTmp = topicOutName
    if not topicOutName:
        topicOutNameTmp = '/ism/' + strParts[2] + '/' + strParts[3] + '/' + strParts[4] + '/'
    # DICTIONARY IS USED: PUBLISHER IS RETURNED USING TOPIC INPUT NAME.
    pubImageObjsDictionary[topics[iTopic]] = rospy.Publisher(topicOutNameTmp, OccupancyGrid, queue_size=1)
...
#            cv2.imwrite("/home/repete/Desktop/TmpImages/image" + str(data.header.seq) + ".png", cv_image);
    ...
    # print Xvis, Yvis
    #tStart = time.time()
    grid, nGridX, nGridY, dist_x1, dist_y1,empty = image2ogm(Xvis,Yvis,cv_image,rHorizon,grid_xSizeInM,grid_ySizeInM,grid_resolution,object_extend)
    ...
    grid_msg.header.frame_id = parent_frame
...
#    print dist_x1, dist_y1
    ...
    if topicOutName:
        pubImageObjsDictionary[topics[0]].publish(grid_msg)
    else:
...
def main():
    for iTopic in range(0,len(topics)): 
        ...
        if not topicOutName:
            topicOutNameTmp = '/ism/' + strParts[2] + '/' + strParts[3] + '/' + strParts[4] + '/'
        print 'image2ism    is subscriping to topic "', topics[iTopic], '" and publishing "', topicOutNameTmp, '"'
