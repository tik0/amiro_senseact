...
void live_demo(LogFileManager& logFileManager, std::string ip)
{
	...
	miniLux.registerListener(dynamic_cast<DataListener<FrameEndSeparator>*>(&allMiniLuxListener));
	miniLux.registerListener(dynamic_cast<DataListener<Scan2208>*>(&allMiniLuxListener));
