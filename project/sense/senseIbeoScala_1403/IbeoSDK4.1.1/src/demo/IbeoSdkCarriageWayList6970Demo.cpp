...
#include <ibeosdk/listener/CarriageWayList6970Listener.hpp>
...
protected:
	VehicleStateBasicEcu m_bufferedVehicleState;
...
void file_demo(const std::string& filename)
{
	...
	if (file.isOpen()) {
		...
		file.registerListener(dynamic_cast<DataListener<RefObjectListEcuEtDyn>*>(&allListener));
		file.registerListener(dynamic_cast<DataListener<VehicleStateBasicEcu>*>(&allListener));
		file.registerListener(dynamic_cast<DataListener<CarriageWayList6970>*>(&allListener));
