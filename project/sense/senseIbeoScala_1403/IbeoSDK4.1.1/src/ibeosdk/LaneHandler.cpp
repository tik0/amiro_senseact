...
LanePosition LaneHandler::getLanePosition(const VehicleStateBasicEcu& point) const
{
	PositionWgs84 zero;
	zero.setLongitudeInDeg(0);
	zero.setLatitudeInDeg(0);
	zero.setCourseAngleInRad(0);

	PositionWgs84 p;
	p.transformFromTangentialPlane(double(point.getXPos())/10000, double(point.getYPos())/10000, zero);
...
LanePosition LaneHandler::getLanePosition(const PositionWgs84& point) const
{
	...
	for (std::vector<CarriageWayPtr>::iterator it = validWays.begin();
		it != validWays.end(); it++)
	{
	...
	for (std::vector<CarriageWaySegmentPtr>::iterator it = validCWSs.begin();
		it != validCWSs.end(); it++)
	{
		for(LaneMap::const_iterator laneIt = (*it)->getLanes()->begin();
			laneIt != (*it)->getLanes()->end(); ++laneIt)
		{
	...
	for (std::vector<LanePtr>::iterator it = validLanes.begin();
		it != validLanes.end(); it++)
	{
		for(LaneSegmentMap::const_iterator segIt = (*it)->getSegments()->begin();
			segIt != (*it)->getSegments()->end(); ++segIt)
		{
			BoundingRectangle mBox = segIt->second->getBoundingBox();
	...
	for (std::vector<LaneSegmentPtr>::iterator it = validLaneSegments.begin(); it != validLaneSegments.end(); it++)
	{
		...
		if (inside && ( std::abs(curLP.lateralDistance) <= curLP.laneSegment->getWidth(curLP.longitudinalDistance)/2 )
