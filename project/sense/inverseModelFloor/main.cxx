...
//    std::vector<uchar> isWeed(scanTf.scan_values().size(), 0);
    std::vector<uchar> isCrop(scanTf.scan_values().size(), 0);
//    std::vector<uchar> isFloor(scanTf.scan_values().size(), 0);

    utils::getLidarEstimations(utils::isFloorId, scanTf.scan_values().data(), isCrop, scanTf.scan_values().size());
    ...
    for (uint rayIdx = 0; rayIdx < isCrop.size(); rayIdx = rayIdx + 1) {
    if (isCrop[rayIdx] == 1) {
      ...
      if (isCrop.size() <= rayIdx + 1) skipNextFullWidthDrawing = isCrop[rayIdx+1] && isCrop[rayIdx+2] ? true : false;
...
//    sendPlot("Plot", imageInformerCrop, isWeed.data(), isCrop.size(), 1, 255, 0, 0);
  }
}
