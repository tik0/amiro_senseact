...
void savingObject(Mat blue, Mat green, Mat red, Mat gray, bool saveImg) {
  ...
  if (saveImg) {
    ...
    bool ok = imwrite(filename, frame);
    if (!ok) {
      std::cout << "Couldn't save object " << (imageId+1) << " with site " << (siteId+1) << " into file '" << directory << savingDir << imageId << "_" << siteId << fileend << "'!\n";
    } 
  }
}
    ...
    for (; ;) {
      ...
      if (!commandQueue->empty()) {
        ...
        // check for save site command
          ...
          if (!saving) {
            printf("No saving in comparison mode possible.\n");
          } else if (siteId >= maxObjectSites) {
            printf("You already saved maximal possible sites!\n");
          } else {
            savingObject(b_hist, g_hist, r_hist, gray_hist, true);
            siteId++;
            newObject = false;
            printf("Saved %i. object %i. site.\n", imageId+1, siteId);
          }
        // check for new object command
        } else if (command == COMMAND_NEW) {
          if (!saving) {
            printf("No saving in comparison mode possible.\n");
          } else if (newObject) {
            printf("There is already a new object created.\n");
          } else if (imageId >= maxObjects) {
            printf("You already saved maximal possible objects!\n");
          } else {
            imageId++;
            siteId = 0;
            newObject = true;
            printf("Created %i. object.\n", imageId+1);
          }
        // check for delete command
        } else if (command == COMMAND_DELETE) {
          imageId = 0;
          siteId = 0;
          printf("All objects unlearned!\n");
