...
void insertObject(boost::shared_ptr<twbTracking::proto::Pose2D> objectPtr) {
	cv::circle(gridmap, cv::Point2f(objectPtr->x()/cellSize,objectPtr->y()/cellSize), objectPtr->orientation()/cellSize, cv::Scalar(0),-1);
	combinedMap = gridmap + edgeMap + edgeMap;
}
...
int main(int argc, char **argv) {
	...
	options.add_options()("help,h", "Display a help message.")("verbose,v", "Print the robots pose and sensorvalues.")(
			...
			po::value<std::string>(&mapFileName), "Load map from file with the given name.")("irin",
			...
			po::value<std::string>(&saveMapInscope), "Scope for filenames to save the map.")("serverScope",
