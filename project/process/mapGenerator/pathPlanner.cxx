...
// calculate the path to the next unknown cell with the dijstra algorithm
void PathPlanner::dijstra(cv::Point2i robotCell, float theta, const cv::Mat &om, std::vector<cv::Point2i> &path, std::vector<cv::Point2i> otherPoses) {
	// macro for quick point access
...
//			cout << repell << endl;
			...
			float totaldistance = access_mat(dist,u) + distance + (float) ((om.at<uchar>(vCell) < 128) ? om.cols + om.rows : 0) /*+ repell */;
