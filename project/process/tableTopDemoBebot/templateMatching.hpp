...
// Prototypes
cv::Rect templateMatching( int match_method, cv::Mat &mTemplate, cv::Mat &mImage  );
...
cv::Rect templateMatching( int match_method, cv::Mat &mTemplate, cv::Mat &mImage )
{
  ...
  rectangle( img_display, matchLoc, Point( matchLoc.x + mTemplate.cols , matchLoc.y + mTemplate.rows ), Scalar::all(255), 2, 8, 0 );
  rectangle( result, matchLoc, Point( matchLoc.x + mTemplate.cols , matchLoc.y + mTemplate.rows ), Scalar::all(255), 2, 8, 0 );
  ...
  return cv::Rect(matchLoc, Point( matchLoc.x + mTemplate.cols , matchLoc.y + mTemplate.rows ));
}
}