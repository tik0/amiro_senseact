...
ts_save_map_pgm(ts_map_t *map, ts_map_t *overlay, char *filename, int width, int height)
{
	...
	for (xp = 0; xp < width; x++, xp++) {
        if (overlay->map[ (map->size - 1 - y) * map->size + x] == 0)
                fprintf(output, "0 ");
            else 
                fprintf(output, "%d ", (int)(map->map[ (map->size - 1 - y) * map->size + x]) >> 8);
	}
	fprintf(output, "\n");
    }
    fclose(output);
...
ts_draw_scan(ts_scan_t *scan, ts_map_t *map, ts_position_t *pos)
{
    ...
    for (i = 0; i != scan->nb_points; i++) {
        if (scan->value[i] != TS_NO_OBSTACLE) {
            ...
            if (x2 >= 0 && y2 >= 0 && x2 < map->size && y2 < map->size)
                map->map[y2 * map->size + x2] = 0;
        }
    }
}
...
ts_draw_scan_RGB(ts_scan_t *scan, ts_map_t *map, ts_position_t *pos, unsigned char *pixmap, int scale, int reversey)
{
    ...
    for (i = 0; i < scan->nb_points; i++) {
        if (scan->value[i] != TS_NO_OBSTACLE) {
            ...
            y2 = (int)floor(pos->y * TS_MAP_SCALE + y2p + 0.5);
            if (x2 >= 0 && y2 >= 0 && x2 < map->size && y2 < map->size) {
                color = map->map[y2 * map->size + x2] >> 8;
                if (reversey)
                    ptr = &pixmap[((map->size - 1 - y2) / scale * map->size / scale + x2 / scale) * 3];
                else
                    ptr = &pixmap[(y2 / scale * map->size / scale  + x2 / scale) * 3];
                ptr[0] = 255 - color;
                ptr[1] = 0;
                ptr[2] = color;
            }
