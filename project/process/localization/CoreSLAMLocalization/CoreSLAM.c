...

{
    ...
    if (x1 < 0 || x1 >= map->size || y1 < 0 || y1 >= map->size)
    ...
    if (x2c >= map->size) {
        if (x1 == x2c) return;
        y2c += (y2c - y1) * (map->size - 1 - x2c) / (x2c - x1);
        x2c = map->size - 1;
    }
    if (y2c < 0) {
        if (y1 == y2c) return;
    ...
    if (y2c >= map->size) {
        if (y1 == y2c) return;
        x2c += (x1 - x2c) * (map->size - 1 - y2c) / (y1 - y2c);
        y2c = map->size - 1;
    }
    ...
    incptrx = (x2 > x1) ? 1 : -1;
    incptry = (y2 > y1) ? map->size : -map->size;
