...
//               computes the occupancy grid map and send it over RSB
//============================================================================
...
const float g_BeBotWidth_m = 0.10;
const float g_BeBotHeight_m = 0.10;
int g_BeBotWidth; // Width in the local map
int g_BeBotHeight;  // Height in the local map
const int g_BeBotNumSensor = 12;
const int g_AMiRoNumSensor = 8;

// Frequence of the publishing
int g_iPeriod_ms = 125;

// The Origin of the the BeBot, which is in the middle of the BeBot,
// and in the middle of the local map
cv::Point BeBotOrigin;

// If the sensor detects an obstacle under this range, the rest of the polygon will be
// drawn as occupied
float g_occStart_m = 0.10;
int g_occStart;  // start in pixel coordinates
...
struct sensor BeBot[g_BeBotNumSensor];

// Prototypes
void sendLocalMap(boost::shared_ptr<std::vector<int> > &messageIr, rsb::Informer< OccupancyGrid2DInt >::Ptr informerOccMap);
...
int main(int argc, char **argv) {
  ...
  options.add_options()("help,h", "Display a help message.")
    ...
    ("period,f", po::value < int > (&g_iPeriod_ms), "Period of the publishing in ms.");
  ...
  g_BeBotWidth = g_BeBotWidth_m / g_fMapLocalResolution_m; // Width in the local map
  g_BeBotHeight = g_BeBotHeight_m / g_fMapLocalResolution_m;  // Height in the local map
  g_occStart = g_occStart_m / g_fMapLocalResolution_m;
  BeBotOrigin = cv::Point( g_uiMapLocalWidth/2, g_uiMapLocalHeight/2);
  ...
  rsb::Informer< OccupancyGrid2DInt >::Ptr informerOccMap = factory.createInformer< OccupancyGrid2DInt > (g_sOutScope_OGM);
  ...
  while (true) {
    ...
    sendLocalMap(myIR.irSensors, informerOccMap);

    // Sleep
    boost::this_thread::sleep( boost::posix_time::milliseconds(g_iPeriod_ms) );
  }
...
void sendLocalMap(boost::shared_ptr<std::vector<int> > &messageIr, rsb::Informer< OccupancyGrid2DInt >::Ptr informerOccMap) {
      ...
      if (messageIr->size() == g_BeBotNumSensor) {
        drawSensorInformation(messageIr, map);
      } else if (messageIr->size() == g_AMiRoNumSensor) {
        // TODO
      }
...
void drawSensorInformation(boost::shared_ptr<std::vector<int> > &messageIr, boost::shared_ptr<OccupancyGrid2DInt> &map) {
  ...
  for (int sensorIdx = 0; sensorIdx < g_BeBotNumSensor; sensorIdx++)
  ...
  rook_points[0][0] = BeBotOrigin + cv::Point(- g_BeBotWidth / 2, g_BeBotWidth / 2);
  rook_points[0][1] = BeBotOrigin + cv::Point(- g_BeBotWidth / 2, - g_BeBotWidth / 2);
  rook_points[0][2] = BeBotOrigin + cv::Point( g_BeBotWidth / 2, - g_BeBotWidth / 2);
  rook_points[0][3] = BeBotOrigin + cv::Point( g_BeBotWidth / 2, g_BeBotWidth / 2);
  const cv::Point* ppt[1] = { rook_points[0] };
  int npt[] = { 4 };
  // Draw the polygon of the sensor
...
void initBeBotSensorCoordinates() {
  ...
  BeBot[0].x = - g_BeBotWidth / 4;
  BeBot[0].y = g_BeBotHeight / 2;
  BeBot[0].value = 10;
  BeBot[1].id = 1;
  BeBot[1].orientation = 90 + 45;
  BeBot[1].x = - g_BeBotWidth / 2;
  BeBot[1].y = g_BeBotHeight / 2;
  BeBot[1].value = 10;
  BeBot[2].id = 2;
  BeBot[2].orientation = 180;
  BeBot[2].x = - g_BeBotWidth / 2;
  BeBot[2].y = g_BeBotHeight / 4;
  BeBot[2].value = 10;
  BeBot[3].id = 3;
  BeBot[3].orientation = 180;
  BeBot[3].x = - g_BeBotWidth / 2;
  BeBot[3].y = - g_BeBotHeight / 4;
  BeBot[3].value = 10;
  BeBot[4].id = 4;
  BeBot[4].orientation = 180 + 45;
  BeBot[4].x = - g_BeBotWidth / 2;
  BeBot[4].y = - g_BeBotHeight / 2;
  BeBot[4].value = 10;
  BeBot[5].id = 5;
  BeBot[5].orientation = 180 + 90;
  BeBot[5].x = - g_BeBotWidth / 4;
  BeBot[5].y = - g_BeBotHeight / 2;
  BeBot[5].value = 10;
  BeBot[6].id = 6;
  BeBot[6].orientation = 180 + 90;
  BeBot[6].x = g_BeBotWidth / 4;
  BeBot[6].y = - g_BeBotHeight / 2;
  BeBot[6].value = 10;
  BeBot[7].id = 7;
  BeBot[7].orientation = 180 + 90 + 45;
  BeBot[7].x =  g_BeBotWidth / 2;
  BeBot[7].y = - g_BeBotHeight / 2;
  BeBot[7].value = 10;
  BeBot[8].id = 8;
  BeBot[8].orientation = 0;
  BeBot[8].x = g_BeBotWidth / 2;
  BeBot[8].y = - g_BeBotHeight / 4;
  BeBot[8].value = 10;
  BeBot[9].id = 9;
  BeBot[9].orientation = 0;
  BeBot[9].x = g_BeBotWidth / 2;
  BeBot[9].y = g_BeBotHeight / 4;
  BeBot[9].value = 10;
  BeBot[10].id = 10;
  BeBot[10].orientation = 45;
  BeBot[10].x = g_BeBotWidth / 2;
  BeBot[10].y = g_BeBotHeight / 2;
  BeBot[10].value = 10;
  BeBot[11].id = 11;
  BeBot[11].orientation = 90;
  BeBot[11].x = g_BeBotWidth / 4;
  BeBot[11].y = g_BeBotHeight / 2;
  BeBot[11].value = 10;
}
