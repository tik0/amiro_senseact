...
int main(int argc, char **argv) {
  ...
  std::string rsbOutScope = "/rir_prox";
  ...
  // Init IRConverter
  IRConverter irConverter(&CAN);

  // Datastructure for the CAN messages
  std::vector<uint16_t> proximityRingValue(irConverter.getIRValueCount(),0);

  int fail = 1;
  uint8_t sensorIdx = 0;
  for(;;) {
    // Read the proximity data
    irConverter.loadIRValues();
    irConverter.getIRValues(proximityRingValue);
    for (sensorIdx = 0; sensorIdx < irConverter.getIRValueCount(); sensorIdx++) {
